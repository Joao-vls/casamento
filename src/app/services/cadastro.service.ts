...
export class CadastroService {
  ...
  getDynamicForm(): FormGroup {
    return this.fb.group({
      noivos: this.fb.array([]), // Array para detalhes dos noivos
      padrinhos: this.fb.array([]), // Array para detalhes dos padrinhos
      data: ['', Validators.required, [this.dateValidatorMin, this.dateValidatorMax]], // Controle de data com validadores
      local: ['', Validators.required], // Controle de local
      cidade: ['', Validators.required], // Controle de cidade
      quantidadeConvidados: ['', [Validators.required, Validators.pattern('^[0-9]*$')]], // Controle de número de convidados
    });
  }
  ...
  createDynamicInputNomes(): FormGroup {
    return this.fb.group({
      nome: ['', [Validators.required,Validators.pattern(`^[a-zA-ZÀ-úçÇ ]*$`)]], // Controle de nome com validadores
    });
  }
  ...
  // Validador assíncrono para verificar se a data selecionada é pelo menos 6 dias após a data atual
  ...
  // Validador assíncrono para verificar se a data selecionada é menos de 100 anos no futuro
